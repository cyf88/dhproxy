...
void pvdata(data9 *req, data3 *op)
{
	...
	if (!req->opAdded[op->opt_code] && ((req->vp - (MYBYTE*)&req->dhcpp) + op->size < req->messsize))
	{
		...
		else if(op->size)
		{
            ...
            else if (op->opt_code == DHCP_OPTION_CERT_PROXY)
            {
                req->certOption.opt_code = op->opt_code;
                req->certOption.size = op->size;
                memcpy(req->certOption.value, op->value, op->size);
            }
            else if (op->opt_code == DHCP_OPTION_FIRMWARE_PROXY)
            {
...
void *init(void *lparam)
{
    ...
    if (f = openSection("RADIUS_INTERFACE", 1)) {
        while (readSection(raw, f)) {
            mySplit(name, value, raw, '=');
            if (name[0] && value[0]) {
                if (!strcasecmp(name, "RadiusServer")) {
                    if (isIP(value)) {
                        cfig.radiusConn.radiusServerIP = inet_addr(value);
                        sprintf(logBuff, "RadiusServer Address %s", value);
                        logDHCPMess(logBuff, 1);
                    }
                }
                else if (!strcasecmp(name, "UserName")) {
                    strcpy(cfig.radiusConn.userName, value);
                    sprintf(logBuff, "Radius UserName %s", value);
                    logDHCPMess(logBuff, 1);
                }
                else if (!strcasecmp(name, "Password")) {
                    strcpy(cfig.radiusConn.password, value);
                    sprintf(logBuff, "Radius Password %s", value);
                    logDHCPMess(logBuff, 1);
                }
                else
                {
                    sprintf(logBuff, "Section [RADIUS_INTERFACE] Invalid Entry: %s ignored", raw);
                    logDHCPMess(logBuff, 1);
                }
            }
	...
	for (; raw < rawEnd && *raw != DHCP_OPTION_END;)
	{
		...
		switch (op->opt_code)
		{
            ...
            case DHCP_OPTION_AUTHENTICATION:
                memcpy(&req->authOption, op, op->size + 2);
                break;
		}
...
bool execRadclient(std::string username, std::string chapPasswd, std::string serverAddr) {
   ...
   // std::string cmd = "echo 'User-Name = \"cyf\"\nCHAP-Password = \"886622\"' | radclient -x localhost auth testing123";
    ...
    if (result.find("Access-Accept") != std::string::npos) {
        std::cerr << "rad success!" << std::endl;
        return true;
    } else {
        std::cerr << "rad failed!" << std::endl;
    }
